...
private fun WikiArticle.body(): List<TextSpan> = when (this) {
    ...
    WikiArticle.NEWS -> listOf(
        ...
        TextSpan("⚠️ Тревожная весть → «Зоркий счёт»: найди числа от 1 до 10 за 10 секунд. Победа вернёт половину урона, проигрыш заморозит доход на 2–3 дня, а ошибка запрёт вывод до самого закрытия. Отступить можно, пока игра не началась\n\n"),
        TextSpan("И бойся «предложения, от которого нельзя отказаться»: лихие люди зарятся на прибыльные дела перед самым их завершением — не досидишь до конца, отдашь половину.")
    )
