...
const PERSONAS: PersonaTemplate[] = [
  ...
  },
  {
    ...
    speechStyle: 'Открыто рассказывает про прошлые провалы — это его фишка. Самоирония и чёрный юмор. Не продаёт — описывает как есть. Не давит и не торопит — нетипично для дельца. Может оказаться единорогом: третий раз у Ивана всегда получается.',
...
export async function generateProjectBanner(
  ...
  // 1152×768 — точное соотношение 3:2 при кратности 64 (SDXL-friendly).
  // Wide aspect'ы (2:1, 7:4) Pollinations не любит — растягивает персонажей.
  const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=1152&height=768&nologo=true&seed=${seed}`
