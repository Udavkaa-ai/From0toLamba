...
export async function computeArchetypeTokens(userId: number): Promise<Record<string, {
  ...
  const [gameState, sessions, projects] = await Promise.all([
  ...
  const result: Record<string, { earned: number; spent: number; balance: number; gamesPlayed: number; dealsTaken: number }> = {}
  ...
  const allArchetypes = new Set<string>([...Object.keys(games), ...Object.keys(deals), ...Object.keys(spent)])
  for (const arch of allArchetypes) {
    const gamesPlayed = games[arch] ?? 0
    const dealsTaken = deals[arch] ?? 0
    const earned = Math.floor(gamesPlayed / TOKENS_PER_GAMES) + Math.floor(dealsTaken / TOKENS_PER_DEALS)
    const archSpent = spent[arch] ?? 0
    const balance = Math.max(0, earned - archSpent)
    result[arch] = { earned, spent: archSpent, balance, gamesPlayed, dealsTaken }
  }
  return result
}
