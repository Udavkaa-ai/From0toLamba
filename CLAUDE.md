...
**Активная версия:** Telegram Mini App (`tg/`) — v1.1.2
...
- **Формула чуйки:** `delta = TP − FP` (без clamp) + бонус `+2` за чистую грамоту (0 подделок, игрок никого не отметил). Применяется к `gameState.intuitionScore` при сабмите. Упущенные подделки (FN) не штрафуют
