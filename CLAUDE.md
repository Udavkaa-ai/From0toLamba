...
**Активная версия:** Telegram Mini App (`tg/`) — v1.5.14
...
- **Difficulty (сила мутации):**
  ...
  - `HARD` (SURVIVOR, UNICORN) — только эмблема в том же классе (медведь↔волк, рыба↔сокол, якорь↔ключ). Точки и стиль ободка в HARD не мутируются — на мелкой SVG под вращением не читаются
