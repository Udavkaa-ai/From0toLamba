...
**Активная версия:** Telegram Mini App (`tg/`) — v1.4.0
...
| Активных дел | max 5 |
| Кулдаун между днями | 3 «быстрых» перехода подряд — без ожидания; после 3-го кнопка блокируется на 2 часа. Крон шлёт уведомление в бот, заглушка «посмотреть рекламу» сбрасывает пачку и открывает ещё 3 быстрых дня |
